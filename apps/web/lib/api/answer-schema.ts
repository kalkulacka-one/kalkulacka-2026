...
/**
 * Wire format for a single answer, as stored in `CalculatorSessionData.answers`
 * on the production database. Must accept exactly what the old app's
 * `@kalkulacka-one/schema` `answerSchema` accepted — other apps still reading
 * this column depend on it.
 */
const sourceSchema = z
  .object({
    url: z.url().describe('URL of a source'),
...
export const answerSchema = z
  .object({
    questionId: z
      .uuid()
      .describe('Unique identifier of an answered question in the format of UUID'),
    answer: z
